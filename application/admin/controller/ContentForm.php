...
class ContentForm extends Controller
{
        ...
        if($string === false){
            ...
            foreach($modelinfo as $val){
                ...
                if($fieldtype == 'input' || $fieldtype == 'number'){
                    ...
                    $string .= $this->tag_start($val['name'],$val['isrequired']).'<input type="text" value="'.$val['defaultvalue'].'" name="'.$val['field'].'" autocomplete="off" class="layui-input'.$required.'" placeholder="'.$val['tips'].'">'.$this->tag_end();
                }elseif($fieldtype == 'textarea'){
                    $errortips = !empty($val['errortips']) ? $val['errortips'] : '必填项不能为空';
                    $required = $val['isrequired'] ? ' required" errortips="'.$errortips : '';
                    $string .= $this->tag_start($val['name'],$val['isrequired']).'<textarea name="'.$val['field'].'" class="layui-textarea'.$required.'" placeholder="'.$val['tips'].'" onKeyUp="textarealength(this,'.$val['maxlength'].')">'.$val['defaultvalue'].'</textarea><p class="textarea-numberbar"><em class="textarea-length">0</em>/'.$val['maxlength'].'</p>'.$this->tag_end();
                }elseif($fieldtype == 'select'){
                    $string .= $this->tag_start($val['name']).'<span class="select-box">'.Form::select($val['field'],$val['defaultvalue'],string2array($val['setting'])).'</span>'.$this->tag_end();
                    ...
                    $string .= $this->tag_start($val['name']).Form::datetime($val['field'], '', string2array($val['setting'])[0]).$this->tag_end();
                }else{
    ...
    public function get_modelinfo() {
        ...
        if($modelinfo === false){
            if(!Db::name('model')->where(['modelid'=>$this->modelid])->find()) showmsg('模型不存在！');
            $modelinfo = Db::name('model_field')->where(['modelid'=>$this->modelid,'disabled'=>0])->order('listorder ASC')->select();
