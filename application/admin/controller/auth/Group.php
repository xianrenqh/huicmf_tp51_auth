...
class Group extends Common
{
    ...
    public function index()
    {
        ...
        $array   = [];
        foreach ($data as $v) {
            $edit_tree = '<a title=\'编辑角色组\' href=\'javascript:;\' onclick=\'WeAdminShow("编辑角色组","'.url('group_edit',
                    ['id' => $v['id']]).'",800)\' style=\'text-decoration:none\'  class=\'layui-btn layui-btn-xs layui-btn-normal\'>编辑角色组</a>';
            if ( ! check_auth('auth.group/group_delete')) {
                $del_tree = '';
            } else {
                $del_tree = '<a title="删除" href="javascript:;" style="text-decoration:none" data-href="'.url('group_delete',
                        ['id' => $v['id']]).'" class="layui-btn layui-btn-xs layui-btn-danger j-tr-del">删除</a>';
            }

            $v['string']   = ($v['id'] == $group_data[0]['id']) ? '' : $edit_tree.$del_tree;
            $v['status']   = $v['status'] == 'normal' ? '<span class="layui-btn layui-btn-xs">启用</span>' : '<span class="layui-btn layui-btn-xs layui-bg-red">禁用</span>';
            $v['parentid'] = $v['pid'];
            $array[]       = $v;
        }
        $str = "<tr>
    ...
    public function group_edit()
    {
        ...
        if (input('dosubmit')) {
            ...
            if ($params) {
                ...
                try {
                    $row->save($params);
                    $children_auth_groups = Db::name('auth_group')->where('id', 'in',
                        implode(',', (Tree2::instance()->init($groupList)->getChildrenIds($row->id))))->select();
                    $childparams          = [];
                    foreach ($children_auth_groups as $key => $children_auth_group) {
                        $childparams[$key]['id']    = $children_auth_group['id'];
                        $childparams[$key]['rules'] = implode(',',
                            array_intersect(explode(',', $children_auth_group['rules']), $rules));
                    }
